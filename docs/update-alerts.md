...
- Kind: bar widget with a popup (`Panel.qml`), helper `bin/omarchy-feedback` (bash).
- Helper commands: `omarchy-feedback update-check [VERSION] [--force]`, `update-dismiss VERSION`, `update-run [all|install]`, all delegating to `lib/update.sh`.
- Version places: `manifest.json` (the helper reads it at run time as `OF_VERSION`), `CHANGELOG.md`.
- Cache: `~/.cache/omarchy-feedback/update-check.json`. Opt-out: `"update_check": false` in `~/.config/omarchy-feedback/config.json`.
- Post-update step: offers `omarchy-feedback daemon restart`, because the recorder keeps running the old code until restarted.
- No mismatch case: `install.sh` symlinks the helper, so the plugin folder is the only copy.
- Tests: `tests/run.sh update` runs the check against `file://` fixtures.
