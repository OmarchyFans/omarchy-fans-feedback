...
# ---- this plugin -------------------------------------------------------------
UPD_ID="fans.omarchy.feedback"
UPD_NAME="Feedback"
UPD_REPO="OmarchyFans/omarchy-fans-feedback"            # GitHub owner/repo the plugin is published from
UPD_BRANCH="main"        # branch whose manifest.json is "the published version"
UPD_SLUG="omarchy-feedback"            # cache lives in ~/.cache/<slug>/update-check.json
UPD_CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/omarchy-feedback/config.json"        # JSON file; "update_check": false turns the check off
...
post_update() {
  # The recorder daemon keeps running the old code until it is restarted.
  local ans
  read -rp "Restart the recorder so it runs the new version? (a moment of events is lost) [Y/n] " ans
  [[ $ans =~ ^[nN] ]] || "$UPD_DIR/bin/omarchy-feedback" daemon restart || true
}
# ------------------------------------------------------------------------------
